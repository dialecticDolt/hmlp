...
np.random.seed(30)
extra = (N%nclasses)
#offset = np.random.rand(d, nclasses) * 30
offset = np.array([[2, 0], [2, 2], [0, 2], [4, 5], [6, 8], [-1, -5], [-1, 6], [6, -1], [10, 10], [0, 10]])
scale = np.random.rand(d, nclasses)
for i in range(nclasses):
    if(extra):
        ...
        split = 0
    new_class = ((np.random.randn(d, (int)(np.floor(N/nclasses))+split) * (np.random.rand()) * 0.25).T + offset[i, :]).T
