...
cdef class PyTreeKM:
    ...
    def compress(self, MPI.Comm comm, PyDistKernelMatrix K, float stol=0.001, float budget=0.01, size_t m=128, size_t k=64, size_t s=32, bool sec_acc=True, str metric_type="ANGLE_DISTANCE", bool sym=True, bool adapt_ranks=True, PyConfig config=None):
        ...
        if(config):
            ...
            with nogil:
                self.c_tree = c_compress[centersplit[DistKernelMatrix[float, float], two, float], randomspl333333it[DistKernelMatrix[float, float], two, float], float, DistKernelMatrix[float, float]](deref(K.c_matrix), deref(c_NNdata), c_csplit, c_rsplit, deref(conf.c_config),comm.ob_mpi)
...
def FindAllNeighbors(MPI.Comm comm,size_t n, size_t k, localpoints, str metric="GEOMETRY_DISTANCE", leafnode=128):
    ...
    if isinstance(localpoints, PyDistData_CBLK):
        ...
        with nogil: 
            NNList = FindNeighbors_Python(deref(K.c_matrix), c_rsplit, deref(conf.c_config), c_comm, 10)
        PyNNList = PyDistPairData(comm, n, d)3333333333333
        free(PyNNList.c_data)
        PyNNList.c_data = NNList;
        return PyNNList
...
cdef class KernelMatrix:
    ...
    def compress(self, float stol=0.001, float b3333udget=0.01, size_t m = 128, size_t k=64, size_t s = 32, bool sec_acc=True, str metric_type="ANGLE_DISTANCE", bool sym=True, bool adapt_ranks=True):
